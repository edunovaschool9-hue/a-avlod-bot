...
async function kimMaktabda(): Promise<string> {
  ...
  const bor = r.filter((x: any) => x.holat === "ichkarida"), yoq = r.filter((x: any) => x.holat !== "ichkarida");
  let t = `🏫 <b>Bugun ishda</b> · ${d.kun}\nDavomat yoki xulosa qilgan: <b>${bor.length}</b> / ${r.length} o‘qituvchi\n`;
  if (bor.length) {
    t += `\n<b>✅ Ishda</b>\n`;
    bor.forEach((x: any) => {
      t += `🟢 ${esc(x.ism)}` + (x.keldi ? ` · ${x.keldi}` : "") + (x.sinflar ? ` · ${esc(x.sinflar)}` : "") +
        (Number(x.xulosa) ? ` · ${x.xulosa} xulosa` : "") + "\n";
    });
  }
  if (yoq.length) {
    t += `\n<b>❓ Hali belgi yo‘q</b>\n` + yoq.map((x: any) => `⚪ ${esc(x.ism)}` + (x.tg ? "" : " <i>(TG yo‘q)</i>")).join("\n") + "\n";
  }
  t += `\n<i>Holat 9:00 dagi sinf davomati va dars xulosalari bo‘yicha aniqlanadi.</i>`;
  return t;
}
...
// ---------- o'qituvchi: fan va sinf tanlash ----------
async function tanlovKor(chat: number, tur: "fan" | "sinf", message_id?: number) {
  const d = await rpc("ep_mening_tanlovim", { p_chat_id: chat });
  if (!d?.ok) { await send(chat, T.royxatda_yoq); return; }
  const list: any[] = (tur === "fan" ? d.fanlar : d.sinflar) ?? [];
  const rows: any[] = [];
  const kk = tur === "fan" ? 2 : 4;
  for (let i = 0; i < list.length; i += kk) {
    rows.push(list.slice(i, i + kk).map((x: any) => ({ text: (x.bor ? "✅ " : "") + x.nom, callback_data: `tn:${tur}:${x.id}` })));
  }
  rows.push(tur === "fan"
    ? [{ text: "➡️ Endi sinflarni tanlash", callback_data: "tn_go:sinf" }]
    : [{ text: "⬅️ Fanlar", callback_data: "tn_go:fan" }, { text: "✅ Tayyor", callback_data: "tn_ok" }]);
  const tanl = list.filter((x: any) => x.bor).map((x: any) => x.nom).join(", ");
  const t = tur === "fan"
    ? `📚 <b>Qaysi fanlarni o‘qitasiz?</b>\nBosib belgilang — qayta bosilsa olib tashlanadi.\n\n<i>Tanlangan: ${esc(tanl || "—")}</i>`
    : `🏫 <b>Qaysi sinflarda dars berasiz?</b>\n\n<i>Tanlangan: ${esc(tanl || "—")}</i>`;
  if (message_id) await tg("editMessageText", { chat_id: chat, message_id, text: t, parse_mode: "HTML", reply_markup: { inline_keyboard: rows } });
  else await send(chat, t, { reply_markup: { inline_keyboard: rows } });
}
async function tanlovCallback(cq: any, k: string, a: string, b: string) {
  const chat = cq.message.chat.id as number, mid = cq.message.message_id;
  const ok = (text = "") => tg("answerCallbackQuery", { callback_query_id: cq.id, text });
  if (k === "tn") { const r = await rpc("ep_tanlov_almash", { p_chat_id: chat, p_tur: a, p_id: Number(b) }); await ok(r?.bor ? "Qo‘shildi" : "Olib tashlandi"); await tanlovKor(chat, a as any, mid); return; }
  if (k === "tn_go") { await ok(); await tanlovKor(chat, a as any, mid); return; }
  if (k === "tn_ok") {
    await ok("Saqlandi");
    const d = await rpc("ep_mening_tanlovim", { p_chat_id: chat });
    const f = ((d?.fanlar ?? []) as any[]).filter((x: any) => x.bor).map((x: any) => x.nom).join(", ");
    const c = ((d?.sinflar ?? []) as any[]).filter((x: any) => x.bor).map((x: any) => x.nom).join(", ");
    await tg("editMessageText", { chat_id: chat, message_id: mid, parse_mode: "HTML",
      text: `✅ <b>Saqlandi</b>\n\nFanlar: <b>${esc(f || "—")}</b>\nSinflar: <b>${esc(c || "—")}</b>\n\nEndi 🧪 <b>Test</b> tugmasida faqat shu sinf va fanlar chiqadi.` });
    return;
  }
  await ok();
}
...
// ---------- belgilanmagan o'quvchilar ----------
async function belgisizYubor(quruq: boolean): Promise<{ yuborildi: number; jami: number; oqit: number }> {
  const d = await rpc("ep_belgisiz", { p_kun: null });
  const sinflar: any[] = (d?.sinflar ?? []) as any[];
  if (!sinflar.length) return { yuborildi: 0, jami: 0, oqit: 0 };
  const h = await rpc("ep_teach_hozir", {});
  const oqit: any[] = ((h?.royxat ?? []) as any[]).filter((x: any) => x.tg);
  const rows: any[] = [];
  for (let i = 0; i < sinflar.length; i += 2) rows.push(sinflar.slice(i, i + 2).map((x: any) => ({ text: `${x.sinf} (${x.soni})`, callback_data: `dv_sinf:${x.sinf_id}` })));
  const royxat = sinflar.map((x: any) => `<b>${esc(x.sinf)}</b> — ${x.soni} ta\n<i>${esc(x.bolalar ?? "")}</i>`).join("\n\n");
  const matn = `⚠️ <b>Davomat belgilanmagan o‘quvchilar</b> · ${d.kun}\n` +
    `Jami: <b>${d.jami}</b> ta o‘quvchi\n\n${royxat}\n\n` +
    `<b>Iltimos javob bering:</b> bu o‘quvchilarni bugun sinfda yoki maktabda ko‘rdingizmi?\n` +
    `Sinfni bosing va <b>keldi</b> yoki <b>kelmadi</b> deb belgilang — ota-onalar javobni kutmoqda.`;
  let n = 0;
  for (const t of oqit) {
    const od = await rpc("ep_odam_chat", { p_id: t.id });
    if (!od?.chat_id) continue;
    if (!quruq) await send(Number(od.chat_id), matn, { reply_markup: { inline_keyboard: rows } });
    n++;
  }
...
async function xabar(msg: any) {
  ...
  if (msg.location) {
    if (!msg.edit_date) await send(chat, "Rahmat, lekin joylashuv endi kerak emas — davomat 9:00 da sinfni tanlash orqali belgilanadi.", { reply_markup: isAdmin ? KB_ADMIN : KB_TEACH });
    return;
  }
  if (false && msg.location) {
...
<i>${esc(matn)}</i>`, { reply_markup: { inline_keyboard: [[{ text: "✅ Yuborish", callback_data: "omm_ok" }, { text: "✖ Bekor", callback_data: "omm_no" }]] } });
  ...
  if (matn === "/maktab") { await send(chat, "Joylashuv nazorati o‘chirilgan. Davomat 9:00 dagi sinf belgilash orqali yuritiladi."); return; }
  ...
  if (/^\S*\s*test$/i.test(matn) || /test$/i.test(matn)) {
    if (!isTeach && !isAdmin) { await send(chat, T.royxatda_yoq); return; }
    let ses: any = await rpc("ep_tg_sessiya", { p_chat_id: chat });
    if (!ses?.token) { const p = await rpc("ep_teach_pin", { p_chat_id: chat }); if (p?.ok) ses = await rpc("ep_kirish", { p_pin: p.pin }); }
    const tok = ses?.token ? String(ses.token) : "";
    await send(chat, "🧪 <b>Kunlik test</b>\n\nSinf → o‘quvchi → fan tanlaysiz, 5 ta savol. Natija ota-onaga va rahbariyatga boradi.\nO‘quvchi ro‘yxatda bo‘lmasa — qo‘shib qo‘yasiz.", {
      reply_markup: { inline_keyboard: [
        [{ text: "🌐 Testni ochish (brauzer)", url: SAYT + "test.html" + (tok ? "#t=" + tok : "") }],
        [{ text: "📱 Telegram ichida", web_app: { url: SAYT + "test.html" + (tok ? "#t=" + tok : "") } }]] } });
    return;
  }
  ...
  if (isAdmin) {
    ...
    if (/reyting/i.test(matn)) {
      const d = await rpc("ep_reyting_tg", { p_chat_id: chat, p_kun: null });
      if (!d?.ok) { await send(chat, "Ruxsat yo‘q"); return; }
      const Q = d.qoida ?? { xulosa: 5, davomat: 5, test: 2 };
      const ball = (x: any, p: string) => Number(x[p + "xul"]) * Q.xulosa + Number(x[p + "dav"]) * Q.davomat + Number(x[p + "test"]) * Q.test;
      const r: any[] = ((d.royxat ?? []) as any[]).map((x: any) => ({ ...x, bk: ball(x, "k_"), bh: ball(x, "h_"), bj: ball(x, "j_") }));
      const kun = [...r].sort((a2: any, b2: any) => b2.bk - a2.bk).filter((x: any) => x.bk > 0);
      const haf = [...r].sort((a2: any, b2: any) => b2.bh - a2.bh).filter((x: any) => x.bh > 0);
      const jam = [...r].sort((a2: any, b2: any) => b2.bj - a2.bj).filter((x: any) => x.bj > 0);
      const med = ["🥇", "🥈", "🥉"];
      const qat = (list: any[], p: string, b: string) => list.map((x: any, i: number) =>
        `${med[i] ?? (i + 1) + "."} <b>${esc(x.ism)}</b> — <b>${x[b]}</b> ball  <i>(${x[p + "xul"]} xulosa · ${x[p + "dav"]} davomat · ${x[p + "test"]} test)</i>`).join("\n") || "—";
      let t = `🏆 <b>O‘qituvchilar reytingi</b> · ${d.kun}\n` +
        `<i>Ball: xulosa ${Q.xulosa} · davomat ${Q.davomat} · test ${Q.test}</i>\n\n` +
        `<b>📅 BUGUN</b>\n${qat(kun, "k_", "bk")}\n\n` +
        `<b>📆 SHU HAFTA</b> (${d.hafta_boshi} dan)\n${qat(haf, "h_", "bh")}\n\n` +
        `<b>🗓 UMUMIY</b> (${d.boshlangan} dan)\n${qat(jam, "j_", "bj")}`;
      const nol = r.filter((x: any) => x.bj === 0);
      if (nol.length) t += `\n\n<b>⚪ Hali faoliyat yo‘q</b>\n` + nol.map((x: any) => `• ${esc(x.ism)}`).join("\n");
      const qq = t.split("\n"); let bb = "";
      for (const q2 of qq) { if ((bb + "\n" + q2).length > 3500) { await send(chat, bb); bb = ""; } bb += (bb ? "\n" : "") + q2; }
      if (bb.trim()) await send(chat, bb);
      return;
    ...
    if (/belgilanmagan/i.test(matn)) {
      ...
      if (!d?.ok) { await send(chat, "Ruxsat yo‘q"); return; }
      const sinflar: any[] = (d.sinflar ?? []) as any[];
      if (!sinflar.length) { await send(chat, `✅ <b>${d.kun}</b> — barcha o‘quvchilar belgilangan.`); return; }
      const t = `⚠️ <b>Davomat belgilanmagan</b> · ${d.kun}\nJami: <b>${d.jami}</b> ta\n\n` +
        sinflar.map((x: any) => `<b>${esc(x.sinf)}</b> — ${x.soni} ta\n<i>${esc(x.bolalar ?? "")}</i>`).join("\n\n");
      const qq = t.split("\n"); let bb = "";
      for (const q2 of qq) { if ((bb + "\n" + q2).length > 3500) { await send(chat, bb); bb = ""; } bb += (bb ? "\n" : "") + q2; }
    ...
    if (/natija/i.test(matn)) {
      ...
      if (!d?.ok) { await send(chat, "Xatolik"); return; }
      const r: any[] = d.royxat ?? [];
    ...
    if (/ota-onalar/i.test(matn)) { await send(chat, "Qaysi ro‘yxat?", { reply_markup: { inline_keyboard: [[{ text: "✅ Botda", callback_data: "ota_bor" }, { text: "❌ Botda emas", callback_data: "ota_yoq" }], [{ text: "🆕 Shartnomasiz murojaatlar", callback_data: "ota_sorov" }], [{ text: "👀 Kim xabarni o‘qidi", callback_data: "ota_oqish" }]] } }); return; }
...
async function callback(cq: any) {
  ...
  if (k === "bz_yubor") {
    ...
    const r = await belgisizYubor(false);
    await send(chat, `📤 <b>${r.yuborildi}</b> ta o‘qituvchiga yuborildi.\nBelgilanmagan: ${r.jami} ta o‘quvchi.`);
    return;
  }
  ...
  if (k === "ota_oqish") {
    await ok();
    const d = await rpc("ep_ota_oqish_tg", { p_chat_id: chat, p_kun: null });
    if (!d?.ok) { await send(chat, "Ruxsat yo‘q"); return; }
    const r: any[] = (d.royxat ?? []).filter((x: any) => Number(x.yuborildi) > 0);
    let t = `👀 <b>Ota-onalar xabarni o‘qidimi</b> · ${d.kun}\nYuborildi: <b>${d.yuborildi}</b> · tasdiqlandi: <b>${d.oqildi}</b>\n`;
    if (!r.length) t += "\nBugun xabar yuborilmagan.";
    else {
      const oq = r.filter((x: any) => Number(x.oqildi) > 0), yoq = r.filter((x: any) => !Number(x.oqildi));
      if (oq.length) t += `\n<b>✅ O‘qigan (${oq.length})</b>\n` + oq.map((x: any) => `• ${esc(x.ism)} — ${x.oqildi}/${x.yuborildi}` + (x.bolalar ? ` <i>${esc(x.bolalar)}</i>` : "")).join("\n") + "\n";
      if (yoq.length) t += `\n<b>⚪ Tasdiqlamagan (${yoq.length})</b>\n` + yoq.map((x: any) => `• ${esc(x.ism)}` + (x.tel ? ` · ${esc(x.tel)}` : "")).join("\n");
    }
    t += `\n\n<i>Telegram o‘qilganini bildirmaydi — bu ota-ona «✅ O‘qidim» tugmasini bosgani.</i>`;
    const qatorlar = t.split("\n"); let buf = "";
    for (const q of qatorlar) { if ((buf + "\n" + q).length > 3500) { await send(chat, buf); buf = ""; } buf += (buf ? "\n" : "") + q; }
    if (buf.trim()) await send(chat, buf);
    return;
  }
  if (k === "ota_sorov") {
...
async function navbatYubor(): Promise<{ teach: number; ota: number; xato: number }> {
  ...
  for (const bot of ["teach", "ota"] as const) {
    ...
    for (const m of list) {
      let kb: any = m.tugmalar ? { reply_markup: m.tugmalar } : {};
      if (bot === "ota" && !m.tugmalar && /^(dav|xul|test):/.test(String(m.kalit ?? ""))) {
        kb = { reply_markup: { inline_keyboard: [[{ text: "✅ O‘qidim", callback_data: `rd:${m.kalit}` }]] } };
      }
      const r = await send(Number(m.chat_id), m.matn, kb, token);
...
Deno.serve(async (req) => {
  ...
  if (q("savol_toldir") !== null) {
    ...
    const KEY = Deno.env.get("DEEPSEEK_API_KEY") ?? "";
    if (!KEY) return jsonc({ ok: false, xato: "kalit yo‘q" });
    const bosh = await rpc("ep_savol_boshliq", { p_token: b.token, p_limit: Math.min(Number(b.limit ?? 4), 6) });
    const ish: any[] = (bosh?.royxat ?? []) as any[];
    if (!ish.length) return jsonc({ ok: true, tugadi: true, qoshildi: 0 });
    let jami = 0; const natija: any[] = [];
    for (const w of ish) {
      const til = w.til === "ru" ? "ru" : "uz";
      const soni = 12;
      const prompt = til === "ru"
        ? `Ты методист школы в Узбекистане. Предмет: ${w.fan}. Класс: ${w.daraja}. Составь ${soni} тестовых вопросов по программе этого класса.\nТребования: вопросы и ответы на русском; 4 варианта, верный один; неверные правдоподобные; верный ответ равномерно среди a,b,c,d; без повторов; факты точные.\nВерни только JSON-массив:\n[{"savol":"...","a":"...","b":"...","c":"...","d":"...","togri":"a","izoh":"кратко"}]`
        : `Sen O‘zbekiston maktabi metodistisan. Fan: ${w.fan}. Sinf: ${w.daraja}. Shu sinf dasturi bo‘yicha ${soni} ta test savoli tuz.\nTalablar: savol va javoblar o‘zbek tilida (lotin); 4 ta variant, faqat bittasi to‘g‘ri; noto‘g‘ri variantlar ishonarli; to‘g‘ri javob a,b,c,d orasida teng; takrorlanmasin; faktlar aniq bo‘lsin.\nFaqat JSON massiv qaytar:\n[{"savol":"...","a":"...","b":"...","c":"...","d":"...","togri":"a","izoh":"qisqa"}]`;
      try {
        const rr = await fetch("https://api.deepseek.com/chat/completions", { method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${KEY}` },
        ...
        if (Array.isArray(arr) && arr.length) {
          const r = await rpc("ep_savol_qosh_tok", { p_token: b.token, p_fan_id: w.fan_id, p_daraja: w.daraja, p_savollar: arr, p_til: til });
          jami += Number(r?.qoshildi ?? 0);
          natija.push({ fan: w.fan, daraja: w.daraja, til, qoshildi: r?.qoshildi ?? 0 });
        }
      } catch (_e) { natija.push({ fan: w.fan, daraja: w.daraja, til, xato: true }); }
    }
    return jsonc({ ok: true, qoshildi: jami, ishlar: natija, qolgan: ish.length });
  }
  ...
  if (q("fan_sora") !== null) {
    const bb = await req.json().catch(() => ({})) as any;
    const aa = await rpc("ep_admin_tekshir", { p_token: bb.token ?? "" });
    if (!aa?.ok && !(CRON && q("fan_sora") === CRON)) return no();
    const h = await rpc("ep_teach_hozir", {});
    const oqit: any[] = ((h?.royxat ?? []) as any[]).filter((x: any) => x.tg);
    let n = 0;
    for (const t of oqit) {
      const od = await rpc("ep_odam_chat", { p_id: t.id });
      if (!od?.chat_id) continue;
      if (!QURUQ) await send(Number(od.chat_id),
        "📚 <b>Fan va sinflaringizni belgilang</b>\n\nHurmatli " + esc(t.ism) + ", test tizimi ishga tushdi.\n\n" +
        "Iltimos, <b>o‘zingiz o‘qitadigan fanlarni</b> va <b>dars beradigan sinflarni</b> belgilang — shunda testda faqat sizning fan va sinflaringiz chiqadi.",
        { reply_markup: { inline_keyboard: [[{ text: "📚 Belgilash", callback_data: "tn_go:fan" }]] } });
      n++;
    }
    return jsonc({ ok: true, yuborildi: n, quruq: QURUQ });
  }
  if (q("hujjat") !== null) {
    const b = await req.json().catch(() => ({})) as any;
  ...
  if (q("belgisiz") !== null) {
    if (!(await cronOk(q("belgisiz")))) return no();
    const r = await belgisizYubor(QURUQ);
  ...
  if (q("oqit_xulosa") !== null) {
    ...
    const d = await rpc("ep_adminlar_chat", {});
    for (const c of (Array.isArray(d) ? d : [])) { if (!QURUQ) await send(Number(c), "🕧 <b>Kunlik hisobot · o‘qituvchilar</b>\n\n" + t); n++; }
    return jsonc({ ok: true, yuborildi: n });
  }
  if (q("admin") !== null) {
    const b = await req.json().catch(() => ({})) as any;
