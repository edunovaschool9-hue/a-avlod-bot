...
async def cmd_students(message: types.Message):
    ...
    if not students:
        text = "\U0001f4ed Hozircha o'quvchilar yo'q."
    else:
        text = f"\U0001f465 <b>O'quvchilar ro'yxati ({len(students)} ta):</b>\n\n"
...
async def delete_student_help(callback: CallbackQuery):
    ...
    for s in students[:10]:
        ...
        label = f"\U0001f5d1 {s['full_name']} (@{username})"
...
async def cmd_add_student(message: types.Message, bot: Bot):
    ...
    await message.answer(
        ...
        f"\U0001f42e Buzoqcha: <b>40 kg</b>\n"
        f"\U0001f4da 1-dars testi ochildi\n\n"
...
async def cmd_warn(message: types.Message, bot: Bot):
    ...
    try:
        msg = (
            ...
            f"Sabab: {reason}\n"
            f"\U0001f42e Buzoqcha -{kg_penalty} kg yo'qotdi"
        )

        if som_penalty:
            msg += f"\n\U0001f4b0 -{som_penalty:,} so'm jarima"
...
async def cmd_add_bytes(message: types.Message, bot: Bot):
    ...
    if len(parts) < 3:
        await message.answer("<code>/add_bytes @username 50</code>")
        return
    ...
    await message.answer(
        f"\u2705 {student['full_name']} ga +{amount} bayt qo'shildi!"
    )

    try:
        await bot.send_message(
            student['telegram_id'],
            f"\U0001f4be <b>+{amount} bayt</b> qo'shildi!\n"
            f"Ustoz tomonidan bonus!"
        )
    except Exception:
        pass
...
async def cmd_sub_bytes(message: types.Message, bot: Bot):
    ...
    if len(parts) < 3:
        await message.answer("<code>/sub_bytes @username 50</code>")
        return
    ...
    await message.answer(
        f"\u2705 {student['full_name']} dan -{amount} bayt ayirildi!"
    )

    try:
        await bot.send_message(
            student['telegram_id'],
            f"\U0001f4be <b>-{amount} bayt</b> ayirildi.\n"
            f"Ustoz tomonidan."
        )
    except Exception:
        pass
    ...
    try:
        if next_lesson:
            await bot.send_message(
                ...
                f"\U0001f4dd <b>{next_lesson['title']}</b>\n"
                f"<i>{next_lesson['text']}</i>\n\n"
                f"Akademiyani oching va davom eting! \U0001f680"
            )
        else:
            await bot.send_message(
                student_id,
                f"\U0001f3c6 <b>Zo'r! Barcha tez aytishlarni tugatdingiz!</b>\n\n"
...
async def taz_reject(callback: CallbackQuery, bot: Bot):
    ...
    try:
        await bot.send_message(
            student_id,
            f"\U0001f504 <b>Tez aytishni qayta yozing!</b>\n\n"
            f"\u274c {lesson_id}-dars ustoz tomonidan rad etildi.\n\n"
            f"\U0001f4dd <b>{lesson['title'] if lesson else f'{lesson_id}-dars'}</b>\n"
            f"<code>{lesson['text'] if lesson else ''}</code>\n\n"
            f"\U0001f4a1 <i>{lesson['hint'] if lesson else ''}</i>\n\n"
            f"Akademiyani oching va qayta yuboraring! \U0001f3a4"
        )
    except Exception:
        pass
